...
| Feature: add existing users only  | **config.yaml**: `features.addExistingUsersOnly`<br>**argument**: `--addexistingusersonly`<br>**env var**: `GOTS_ADDEXISTINGUSERSONLY` | Only add users to teams if they already exist in Grafana; do not create missing users (they are skipped)<br><br>**Type**: `bool`<br>**Default**: `false` |
